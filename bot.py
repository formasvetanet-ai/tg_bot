...
CONSULTATION_QUESTIONS = [
    ...
    "3️⃣ Что у вас уже есть?\n(ТЗ / Контакты поставщиков / Коммерческие предложения / Данные по весу и габаритам / Бюджет / Пока ничего нет)",
...
async def send_collected_data(context, user, section_key, answers, ai_response=None):
    ...
    if section_key == "branding":
        ...
        try:
            ...
            await context.bot.send_message(
                chat_id=ROMAN_ID,
                text=f"⚠️ Не удалось отправить в @promo_merch. Перешли им вручную:\n\n{branding_text}"
            )
