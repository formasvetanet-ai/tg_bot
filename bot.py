...
async def send_collected_data(context, user, section_key, answers):
    ...
    text = f"📥 НОВАЯ ЗАЯВКА — {section_name}\n\n"
    text += f"От: {user.first_name}"
    if user.username:
    ...
    for i, answer in enumerate(answers):
        if i < len(questions):
            q = questions[i].split(":")[0]
            text += f"{q}:\n{answer}\n\n"

    await context.bot.send_message(chat_id=ROMAN_ID, text=text)
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "consult":
        ...
        step = context.user_data.get("step", 0)
        keyboard = get_keyboard_for_step("consultation", step)
        await query.message.reply_text(
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔎 Поиск поставщика":
        ...
        await update.message.reply_text(
            ...
            "Подробнее об услуге: @Akhmedzyanov_ru\n\n"
