...
DELIVERY_QUESTIONS = [
    ...
    "8️⃣ Стоимость товара в USD (нужна для расчёта таможенного оформления).\nБез неё будет считаться только логистика. Если не знаете — пропустите:",
    "9️⃣ Ваше имя:",
...
async def send_collected_data(context, user, section_key, answers, ai_response=None):
    ...
    for attempt in range(3):
        try:
            ...
            import traceback
            print(f"Ошибка записи в таблицу (попытка {attempt+1}): {e}")
            if attempt == 2:
                await context.bot.send_message(
                    chat_id=ROMAN_ID,
                    text=f"⚠️ Ошибка таблицы после 3 попыток: {e}"
                )
            else:
                import asyncio
